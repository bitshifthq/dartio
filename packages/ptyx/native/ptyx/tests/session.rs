...
use bytes::Bytes;
use ptyx::{Event, Runtime, Size, SpawnError, SpawnOptions, WriteErrorKind};
...
fn rejected_write_accepts_no_partial_prefix() {
    ...
    let bytes = Bytes::from(vec![b'a', b'b']);
    let original_pointer = bytes.as_ptr();
    let rejected = session.write(bytes);
    let accepted = session.write(Bytes::from_static(b"z"));

    let rejected = rejected.expect_err("oversized write must be rejected");
    assert_eq!(rejected.kind(), WriteErrorKind::Backpressure);
    let recovered = rejected.into_bytes();
    assert_eq!(recovered.as_ptr(), original_pointer);
    assert_eq!(recovered, Bytes::from_static(b"ab"));
