...
capabilities and lifecycle guarantees. Delayed input, output, query,
infrastructure, and cleanup failures retain their stable operation, category,
and native status in `OperationError`. Size, process-id, terminal-mode, and
terminal-name access are independent queries; the crate does not promise an
atomic cross-field metadata snapshot. A rejected write also returns the exact
