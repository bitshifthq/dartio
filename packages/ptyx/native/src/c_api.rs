use bytes::Bytes;
use ptyx_engine::{BrokerSpawn, Failure, FailureKind, IntegratedRuntime, Notice};
...
pub(crate) const STATUS_INVALID_ARGUMENT: u32 = 1;
pub(crate) const STATUS_STALE_HANDLE: u32 = 2;
pub(crate) const STATUS_WRONG_STATE: u32 = 3;
...
pub(crate) const STATUS_INTERNAL: u32 = 10;
const STATUS_BUFFER_TOO_SMALL: u32 = 11;

const ERROR_DOMAIN_NONE: u32 = 0;
pub(crate) const ERROR_DOMAIN_ARGUMENT: u32 = 1;
pub(crate) const ERROR_DOMAIN_STATE: u32 = 2;
const ERROR_DOMAIN_INPUT: u32 = 3;
const ERROR_DOMAIN_OUTPUT: u32 = 4;
pub(crate) const ERROR_DOMAIN_RUNTIME: u32 = 6;
const ERROR_DOMAIN_OS: u32 = 7;

const ERROR_NONE: u32 = 0;
pub(crate) const ERROR_INVALID_ARGUMENT: u32 = 1;
pub(crate) const ERROR_STALE_HANDLE: u32 = 2;
pub(crate) const ERROR_WRONG_STATE: u32 = 3;
const ERROR_QUEUE_FULL: u32 = 4;
const ERROR_UNSUPPORTED: u32 = 5;
const ERROR_CLOSED: u32 = 6;
const ERROR_NATIVE_FAILURE: u32 = 7;
pub(crate) const ERROR_INFRASTRUCTURE_LOST: u32 = 8;

const OPERATION_NONE: u32 = 0;
pub(crate) const OPERATION_RUNTIME_CREATE: u32 = 1;
pub(crate) const OPERATION_RUNTIME_SHUTDOWN: u32 = 2;
const OPERATION_SPAWN: u32 = 3;
const OPERATION_WRITE: u32 = 4;
pub(crate) const OPERATION_OUTPUT: u32 = 5;
const OPERATION_RESIZE: u32 = 6;
const OPERATION_TERMINATE: u32 = 7;
const OPERATION_METADATA: u32 = 8;
pub(crate) const OPERATION_CLOSE: u32 = 9;
...
pub(crate) struct Error {
    ...
    pub(crate) native_code: i32,
    pub(crate) flags: u32,
    pub(crate) context: [u64; 2],
    pub(crate) reserved: [u64; 3],
}

impl Error {
    pub(crate) fn value(domain: u32, kind: u32, operation: u32, native_code: i32) -> Self {
        Self {
...
impl Event {
    pub(crate) fn empty(struct_size: u32) -> Self {
        Self {
            struct_size,
            kind: 0,
            flags: 0,
...
impl<T> Registry<T> {
    ...
    #[cfg(feature = "test-controls")]
    pub(crate) fn sole(&self) -> Option<&T> {
...
struct AdapterState {
    ...
    sessions: Registry<Arc<SessionEntry>>,
...
fn session_entry(handle: u64) -> Option<Arc<SessionEntry>> {
    let state = adapter().lock().ok()?;
    state.sessions.get(handle).map(Arc::clone)
}
...
pub(crate) unsafe fn error_is_valid(error: *mut Error) -> bool {
    error.is_null() || (*error).struct_size as usize >= size_of::<Error>()
}

pub(crate) unsafe fn clear_error(error: *mut Error) {
    if !error.is_null() && (*error).struct_size as usize >= size_of::<Error>() {
        *error = Error::none();
    }
}

pub(crate) unsafe fn set_error(error: *mut Error, value: Error) {
    if !error.is_null() && (*error).struct_size as usize >= size_of::<Error>() {
        *error = value;
    }
}
...
unsafe fn runtime_broker_path(options: *const RuntimeOptions) -> Result<PathBuf, ()> {
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    {
        crate::broker_materializer::broker_path().map_err(|_| ())
    }
    #[cfg(windows)]
    {
        Ok(PathBuf::new())
    ...
    boundary(error, || {
        ...
        *session = 0;
        ...
        match runtime.engine.spawn_start_notified(
            ...
            Err(failure) => {
                if let Ok(mut state) = adapter().lock() {
                    state.sessions.remove(public_handle);
                }
...
pub unsafe extern "C" fn ptyx_session_write(
    ...
    boundary(error, || {
        ...
        let (entry, engine_handle) = match require_active(session, OPERATION_WRITE, error) {
            Ok(value) => value,
...
#[cfg(feature = "test-controls")]
pub(crate) fn test_delay_next_spawn(milliseconds: usize) {
    TEST_SPAWN_DELAY_MS.store(milliseconds, Ordering::Release);
}

#[cfg(feature = "test-controls")]
pub(crate) fn test_spawn_delay_active() -> bool {
    TEST_SPAWN_DELAY_ACTIVE.load(Ordering::Acquire)
}

#[cfg(feature = "test-controls")]
pub(crate) fn test_fail_next_write() {
    TEST_WRITE_INFRASTRUCTURE_FAILURE.store(true, Ordering::Release);
}

#[cfg(feature = "test-controls")]
pub(crate) fn test_kill_broker(runtime: u64) -> bool {
    let Some(runtime) = runtime_entry(runtime) else {
        return false;
    };
    #[cfg(any(target_os = "linux", target_os = "macos"))]
...
mod tests {
    use super::{
        decode_handle, Error, Event, Registry, RuntimeOptions, SessionSnapshot, SpawnOptions,
    };
    use std::mem::size_of;
